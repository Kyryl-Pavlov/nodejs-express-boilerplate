<div align="center">
  <img src="docs/favicon.png" width="80" height="80" alt="Node.js Express Boilerplate"/>
</div>

# nodejs-express-boilerplate

A production-ready **Node.js / TypeScript / Express** boilerplate for building scalable **REST** and **GraphQL** APIs — engineered for teams who want a clean architecture without spending weeks on infrastructure.

Ships fully wired: **JWT authentication** (`jsonwebtoken`), **PostgreSQL** with Prisma ORM and migrations, **AWS S3** file uploads with presigned URLs, async event processing via **AWS SQS + Lambda**, **Redis** caching, **Nginx** reverse proxy with DDoS protection, structured logging to **Sentry / CloudWatch / Loki**, **Prometheus** metrics with **Grafana** dashboards, **Docker Compose** full-stack setup, and a **Vitest** test suite (unit · integration · E2E) — all production-wired from the first commit.
...
- REST API (Express routers, versioned at `/api/v1/`)
- GraphQL API (`graphql-http`, SDL-first, at `/graphql`)
- JWT authentication (access + refresh tokens, via `jsonwebtoken`)
- PostgreSQL with Prisma — migrations auto-generated and applied on every `docker compose up --build`
- S3 media uploads (LocalStack for local dev, real AWS in production)
- Async event processing via **SQS + Lambda** — the app publishes events to SQS; a Lambda function (locally: a worker container) consumes them and writes to Postgres
...
- Prometheus metrics endpoint (`/metrics`) with per-route request duration histograms
...
- VSCode debugger integration (Docker attach + host launch, via Node's built-in inspector)
- Git hooks for automatic code formatting and linting (ESLint + Prettier, via Husky/lint-staged)
- npm workspaces monorepo (`services/app`, `lambda`) with a shared TypeScript/lint/test toolchain

---

## How It Works

Every feature is exposed over **both REST and GraphQL**. Both share the same Prisma models and service modules — only the transport layer differs. You can use whichever style fits your client best.
...
**Media uploads** are stored in S3 (or LocalStack locally). The database only stores the S3 object key — never a URL. When you need to display a file, the API generates a **presigned URL** on demand: a time-limited, signed link that grants direct browser access to the file without exposing your S3 credentials. Presigned URLs expire after 24 hours by default (GraphQL) — the REST upload endpoint returns a hardcoded 1-hour `expires_in` value, an inconsistency carried over from the original boilerplate on purpose.
...
│       │   ├── api/
│       │   │   └── utils/           # restApiResponse()
│       │   ├── routes/
│       │   │   └── v1/              # REST route modules (auth, media, events, cacheTest, health)
│       │   └── graphql/
│       │       ├── resolvers/       # GraphQL mutations and queries
│       │       ├── schema.ts        # SDL type definitions
│       │       ├── multipartUpload.ts  # hand-rolled GraphQL multipart upload parsing
│       │       ├── response.ts      # makeResponse() — the GraphQL response envelope
│       │       └── utils.ts         # Shared GraphQL helpers (model → payload converters)
│       ├── prisma/
│       │   ├── schema.prisma        # User, Media, Event models
│       │   └── migrations/          # Generated SQL migrations
│       ├── Dockerfile               # Production image (multi-stage, plain `node`)
│       └── Dockerfile.dev           # Dev image (tsx watch + Node inspector)
├── lambda/
│   ├── src/
│   │   ├── handler.ts       # Lambda entry point: handler(event, context)
│   │   ├── poll.ts          # Long-poll loop entry point, for local dev
│   │   ├── processRecord.ts # Shared record-processing logic
│   │   ├── db.ts            # Prisma client + Secrets Manager DATABASE_URL resolution
│   │   └── safeExc.ts       # Inline traceback sanitizer (mirrors app's dataFilter)
│   ├── Dockerfile           # Local dev — long-polling worker (used by docker-compose)
│   ├── Dockerfile.lambda    # AWS Lambda — uses Lambda Node.js base image
│   └── package.json         # Lambda dependencies (AWS SDK v3, @prisma/client)
...
> **Why the root `package.json`, `tsconfig.base.json`, and each workspace's manifest all matter to the Docker build:** build context is anchored at `.` so all Dockerfiles can reach the root files plus every workspace's `package.json` (npm workspaces installs need every workspace's manifest present, even when building a single service's image).
...
If you want to run the app directly on your machine (debug Option 2), you also need:

| Tool        | Version | Purpose                                | Download                                                |
| ----------- | ------- | -------------------------------------- | ------------------------------------------------------- |
| **Node.js** | 22+     | Runs the app and installs dependencies | [nodejs.org](https://nodejs.org/)                       |
...
> `APP_ENV` (`development`/`production`) is set directly in `docker-compose.yml`'s `app` service and `.env.ci` — you don't normally need it in `.env.local`.
...
> `AWS_SQS_ENDPOINT_URL` and `SQS_QUEUE_URL` are injected by Docker Compose automatically for the `app` and `worker` containers. Add them to `.env.local` only when running the app directly on the host (debug Option 2), replacing `localstack` with `localhost`.
...
| Express API (direct)      | http://localhost:5000         | Bypass Nginx for debugging                                        |
| GraphQL Explorer (`ruru`) | http://localhost/graphql      | Interactive query UI (dev only — gated by `graphqlIntrospection`) |
...
> Code changes are reflected immediately without rebuilding — the source directory is mounted as a volume and `tsx watch` restarts the process on file changes.
...
An interactive GraphQL Explorer (powered by [`ruru`](https://www.npmjs.com/package/ruru), served via `ruruHTML()`) is available at http://localhost/graphql in development — it's only mounted when `graphqlIntrospection` is `true` (see [Security](#security)). The same flow works over GraphQL using mutations and queries.

> For authenticated requests in the GraphQL Explorer, add an HTTP header: `{"Authorization": "Bearer <access_token>"}`
...
File uploads over GraphQL use the [multipart request spec](https://github.com/jaydenseric/graphql-multipart-request-spec). Use a client that supports it (e.g. Postman, or Apollo Client with the upload link):
...
> GraphQL field names use **camelCase** (`accessToken`, `mediaId`, `expiresIn`) even though REST uses snake_case — the SDL is hand-written camelCase directly, since (unlike Strawberry for Python) a hand-written SDL schema executed by `graphql-http` has no automatic field-casing conversion step.

---

## Database Migrations

Migrations are **automatic** — every `docker compose up --build` runs `prisma migrate dev` (dev) or `prisma migrate deploy` (CI), applying any pending migrations against `services/app/prisma/schema.prisma`. You never need to run migration commands manually for new models, but you do need to **generate** a migration file after changing the schema (Prisma, unlike some ORMs, won't auto-generate migrations on every code change — only `migrate dev` does that, interactively).

To generate and apply a new migration after editing `schema.prisma`:

```bash
bash migrate.sh
# or, without the helper script:
docker compose exec app npx prisma migrate dev --schema services/app/prisma/schema.prisma
```

> Use `exec app` (not `run --rm migrate`) so the generated migration file is written to the host via the volume mount.

> New models must be added to `services/app/prisma/schema.prisma` — there's no separate model-registration step the way SQLAlchemy needed (Prisma reads the whole schema file directly).

---

## Reverse Proxy & Load Balancer

All external traffic enters through **Nginx on port 80** (`nginx/nginx.conf`). The Express app is not exposed publicly — Nginx proxies to it internally at `app:5000`.
...
The `worker` container (`lambda/src/poll.ts`) polls SQS using long-polling (`WaitTimeSeconds=5`). When a message arrives it inserts a row into the `events` table using Prisma's `createMany({ skipDuplicates: true })` (translates to `ON CONFLICT DO NOTHING` on `sqs_message_id`) — safe for at-least-once delivery. Messages are deleted from the queue only after a successful DB write.

**Deploying to AWS:**

`lambda/src/handler.ts` already exports `handler(event, context)` — the Lambda entry point. Two Dockerfiles handle the two runtimes:

| File                       | Used by                                    | Runtime                                                                           |
| -------------------------- | ------------------------------------------ | --------------------------------------------------------------------------------- |
| `lambda/Dockerfile`        | `docker-compose.yml`                       | Long-polling loop (`poll()`) — blocks and polls SQS continuously, via `tsx watch` |
| `lambda/Dockerfile.lambda` | `deploy-dev.yml` / `deploy-prod.yml` CI/CD | AWS Lambda Node.js base image — Lambda runtime invokes `handler()` per SQS batch  |

The CI/CD pipeline builds `Dockerfile.lambda` and pushes it to ECR. On AWS, `DATABASE_URL` points to RDS, `SQS_QUEUE_URL` to the real queue, and `AWS_SQS_ENDPOINT_URL` is unset (the AWS SDK routes to real AWS automatically).

> For high-concurrency Lambda deployments, add **RDS Proxy** in front of your database to multiplex connections. `handler()` disconnects its Prisma client after every invocation so each cold/warm start doesn't accumulate lingering connections, mirroring the original's `NullPool` intent.
...
If `REDIS_URL` is not set, `app.cache` is `null` and all cache calls are skipped — the app degrades gracefully without errors.
...
**File uploads are restricted** — only `jpg`, `jpeg`, `png`, `gif`, `webp`, `pdf`, `mp4`, `mov` are accepted. Anything else returns `415`. Maximum upload size is 50 MB, enforced by a content-length guard middleware plus `express.json()`'s own `limit` option. To extend the allowlist, edit `ALLOWED_EXTENSIONS` in [services/app/src/routes/v1/media.ts](services/app/src/routes/v1/media.ts).

**SQL/connection-string details are stripped from logs** — database errors can include the query and connection details in their string representation. `sanitizeTraceback()` in [services/app/src/logging/dataFilter.ts](services/app/src/logging/dataFilter.ts) redacts `[SQL: ...]`, `[parameters: ...]`, and connection strings from every traceback before it reaches any log backend (Console, Sentry, CloudWatch, Loki). The Lambda worker has an equivalent `safeExc()` helper.

**GraphQL introspection is disabled in production** — controlled by `graphqlIntrospection` in `config.ts`, `true` only when `configName === "development"`. Introspection lets unauthenticated callers enumerate your entire schema.

**JWT algorithm is pinned** — signed and verified via `jsonwebtoken` with an explicit `algorithm: "HS256"`, plus a custom `type: "access" | "refresh"` claim checked on every verify, preventing a refresh token from being used where an access token is required (and vice versa).
...
**Content-Security-Policy is not set** — a CSP covering the GraphQL Explorer requires tuning per deployment. Add it once you know your frontend's allowed origins.
...
| Non-root containers                   | Express app runs as `app` system user; Lambda runs as `nobody` — no root access if a container is compromised                          |
| ECS task role scoped to send-only SQS | Express app can only `SendMessage`. Only the Lambda worker can `ReceiveMessage` and `DeleteMessage`                                    |
...
| MIME type sniffing             | Files go to S3, never executed server-side  | Add server-side content-type sniffing if serving files from a public CDN            |

---

## Code Quality

Git hooks run automatically on every `git commit` and keep the codebase consistently formatted, via Husky + lint-staged. They're installed automatically the first time you run `npm install` (the root `package.json`'s `prepare` script sets them up):
...
1. Runs `eslint --fix` on staged `*.ts` files (import order, unused vars, etc.)
2. Runs `prettier --write` on staged `*.ts`/`*.{json,yml,yaml,md}` files
3. Re-stages any fixed files so they are included in the same commit — no second commit needed
4. Aborts the commit only if `eslint` reports an unfixable error

To run the checks across the entire codebase without committing:
...
ESLint (flat config) and Prettier config live at the repo root — `eslint.config.js` and `.prettierrc` — one config for the whole npm workspace, no per-service duplication.
...
Unit tests are fully isolated (no Docker needed). Integration tests spin up a real Postgres via a `@testcontainers/postgresql` container — this is the one place local dev needs Docker for tests, a disclosed trade-off versus the original Python boilerplate's Docker-free SQLite-in-memory test DB (Prisma has no equivalent transparent SQLite swap without schema-drift risk).

```bash
npm run test:unit          # no Docker needed
npm test                   # unit + integration (Docker needed for the testcontainer)
```
...
└── app/
    ...
    ├── integration/   — supertest against the Express app + a real Postgres testcontainer
    ...
    └── e2e/           — real HTTP through Nginx (CI/CD only)
        └── e2e.spec.ts  health check, full auth flow, S3 upload, SQS event publish
```
...
| **Unit & Integration Tests** | `vitest run tests/app/unit tests/app/integration --coverage` | Yes (integration tier's testcontainer — GitHub-hosted runners have Docker preinstalled) |
| **E2E Tests**                | Spins up `docker-compose.ci.yml`, runs the full e2e suite    | Yes                                                                                     |
...
**Adding a new microservice:** add a build step to `build` with `context: .` and `file: services/<name>/Dockerfile`, a `run_migration` call (`'["npx","prisma","migrate","deploy"]'`) in both `migrate-*` jobs, and a deploy step at the correct tier in both `deploy-*` jobs. Workers go in `deploy-workers-*`.
...
- All secrets (DB password, JWT key, app secret key) are stored in AWS Secrets Manager and injected into containers at startup — never in environment files or task definitions
...
terraform init
terraform apply -var="bucket_name=myorg-nodejs-express-boilerplate-tfstate"
...
#    docker build -f services/app/Dockerfile -t <registry>/nodejs-express-boilerplate-dev/app:latest .
#    docker push <registry>/nodejs-express-boilerplate-dev/app:latest
#    (repeat for lambda/Dockerfile.lambda → /worker:latest, but add --provenance=false --sbom=false —
#     Lambda's CreateFunction rejects the attestation manifest Docker attaches by default)
...
Two debug modes are supported, both configurable in `.vscode/launch.json` (gitignored — create it locally; see `CLAUDE.md` for details on what each mode needs).

### Option 1 — Attach debugger to Docker container

The `app` service runs the Express app under `tsx watch` with Node's built-in inspector (port 9229) and mounts the source directory as a volume, so code changes are reflected immediately without rebuilding.
...
3. Attach VSCode's Node debugger to `localhost:9229` and press **F5**.
...
Run only the infrastructure in Docker and the app directly on your machine. This can give a faster feedback loop.
...
1. Start infrastructure: `docker compose up -d postgres localstack migrate`.
2. Run `npm run dev --workspace=services/app` with `DATABASE_URL`/S3 endpoint vars pointed at `localhost` instead of Docker service hostnames.

**Note on hostnames:** when the app runs on the host it cannot resolve Docker service names like `postgres` or `localstack`. Override these manually:
...
| ---------- | ---------------------- | ----------------------------------------------------------------------------------- |
| Console    | —                      | Always active (via `pino`); debug level in dev, warn in prod                        |
...
All log `data` payloads are automatically filtered by `maskSensitive()` before reaching any backend — sensitive keys (`password`, `token`, `secret`, `authorization`, etc.) are replaced with `***`.
...
| Panel                     | What it shows                                 |
| ------------------------- | --------------------------------------------- |
| Request Rate              | Requests/sec across all routes                |
...
| Request Rate by Route     | Per-route throughput with mean/max table      |
| Latency by Route (p95)    | Per-route p95 with mean/max table             |
...
The Express app exposes a Prometheus-format metrics endpoint at `http://localhost:5000/metrics`, via `prom-client`. Prometheus scrapes it every 15 seconds. Query directly at `http://localhost:9090`.
...
```logql
{app="nodejs-express-boilerplate"}                    # all logs
{app="nodejs-express-boilerplate", level="error"}     # errors only
{app="nodejs-express-boilerplate"} |= "upload"        # text search
```
...
The production Docker image uses `services/app/Dockerfile` (multi-stage build, plain `node`, no dev tooling). To smoke-test it in isolation:
...
# To stop: docker stop nodejs-express-boilerplate-<service>
```
...
# so the AWS SDK routes to real AWS automatically
```
